...
EYE_BLINK_HEIGHT = .25 # threshold may depend on relative face shape

# SIGNIFICANT_BPM_CHANGE = 8

LIP_COMPRESSION_RATIO = .3 # from testing, ~universal
...
def main():
  ...
  # For realtime feed to detect face
  if SECOND:
  ...
  with mp.solutions.face_mesh.FaceMesh(
    ...
    with mp.solutions.hands.Hands(
      ...
      if len(args.input) == 4:
        ...
        # for taking realtime input from webcam
        while cap.isOpened():
...
# function to draw landmarks on the face and hands
...
# function to add text on video
def add_text(image, tells, calibrated):
  global mood

  text_y = TEXT_HEIGHT
  if mood:
    write("Mood: {}".format(mood), image, int(.65 * image.shape[1]), TEXT_HEIGHT)
  if calibrated:
...
# determining the aspect ratio of face based on video dimension
def get_aspect_ratio(top, bottom, right, left):
  height = dist.euclidean([top.x, top.y], [bottom.x, bottom.y])
  width = dist.euclidean([right.x, right.y], [left.x, left.y])
  return height / width

# drawing the eye coordinates 
...
# compare blink between frames to determine rate
def get_blink_tell(blinks):
  if sum(blinks[:RECENT_FRAMES]) < 2: # not enough blinks for valid comparison
    return None
  ...
  if recent_closed > (15 * avg_closed):
    return "Increased blinking"
  elif avg_closed >  (15 * recent_closed):
    return "Decreased blinking"
  else:
    return None


# detect hand over face blocking
...
def get_avg_gaze(face):
  ...
  return round((gaze_left + gaze_right) / 2.5, 1)

# function to get gaze of person based on coordinates
# compare intial and final position of eye
...
# if new gaze varies from the average fixed gaze then change in gaze
...
# return the coordinates of lips from the current aspect ratio of face
...
def get_mood(image):
  ...
  calculating_mood = False
  if score and (score > .38 or detected_mood == 'neutral'):
    mood = detected_mood


# adding the meter image on screen 
def add_truth_meter(image, tell_count):
  ...
  # if probaility of all factors increase move rectangle along image
  if tell_count:
...
# return overall area of face based on input
...
# returning all features linke blink,hand,gaze to determine facial cues
